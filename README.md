...
自动收集、整理和更新电视直播源的Python工具，支持IPv6优先、多源合并、黑名单过滤等功能。

## 功能特点

- ✅ **多源收集**：从多个源URL收集直播源
- ✅ **频道合并**：自动合并同名电视台，支持多源切换
- ✅ **IPv6优先**：IPv6源自动优先排序
- ✅ **黑名单过滤**：自动过滤慢速源和无效源
- ✅ **分类整理**：按央视、卫视、地方台等分类
- ✅ **多版本输出**：生成不同格式的M3U文件
- ✅ **自动更新**：GitHub Actions每天自动更新
- ✅ **详细统计**：生成JSON格式的详细数据
...
channel_processor.py：处理频道名称清理、分类、合并等

speed_tester.py：负责URL速度测试

blacklist_manager.py：管理黑名单的加载和保存

m3u_generator.py：生成各种格式的M3U文件
...
```bash
./setup.sh  # 安装
python scripts/run_collect.py  # 运行
```

2. GitHub自动运行：
- 每天UTC 18:00（北京时间凌晨2点）自动运行

- 可以手动触发

- 提交到sources.txt或代码时自动运行

3. 文件访问：

- 主文件：live_sources.m3u
